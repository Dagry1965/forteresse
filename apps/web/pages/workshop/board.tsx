...
export default function WorkshopBoardPage() {
  const [interventions, setInterventions] = useState<BoardIntervention[]>([]);
  ...
  const fetchInterventions = useCallback(async () => {
    try {
      ...
      setInterventions(Array.isArray(data) ? (data as BoardIntervention[]) : []);
    } catch (e: unknown) {
      const status =
  ...
  const moveCase = async (intervention: BoardIntervention, newStatus: string) => {
    try {
      ...
      const caseId = intervention.case_id || intervention.case?.id;
      if (!caseId) {
  ...
  const getBoardStatus = (intervention: BoardIntervention) =>
    ...
    <div className="p-6 h-[calc(100vh-80px)] flex flex-col bg-slate-50">
      <div className="flex justify-between items-end mb-8">
        <div>
          ...
          <p className="text-slate-500 font-medium">
            Gestion visuelle des dossiers en cours
          </p>
        </div>
      ...
      <div className="flex gap-6 overflow-x-auto pb-6 flex-1 min-h-0 custom-scrollbar">
        {COLUMNS.map((col) => (
          <div
            key={col.id}
            className={`flex-shrink-0 w-80 flex flex-col rounded-xl border border-slate-200 shadow-sm ${col.bg}`}
          >
            ...
            <div className="p-3 space-y-4 overflow-y-auto flex-1">
              {interventions
                ...
                .map((int) => {
                  ...
                  const proformaLocked =
                    (col.id === CASE_STATUS.WAITING_PARTS ||
                      col.id === CASE_STATUS.IN_PROGRESS ||
                  ...
                  return (
                    ...
                    >
                      ...
                      {latestProforma && !proformaAccepted && (
                        <div className="mb-3 rounded-xl border border-amber-200 bg-amber-50 px-3 py-2 text-[11px] font-bold text-amber-700">
                          Devis en attente d&apos;acceptation — progression atelier limitée.
                        </div>
                      )}
