# =====================================================
# Script : Créer le module Invoices (Version stable)
# =====================================================

$backendPath = "C:\Users\Admin\forteresse\apps\backend"
$baseDir = Join-Path $backendPath "src\modules\invoices"
$dtoDir = Join-Path $baseDir "dto"

Write-Host "Création du module Invoices..." -ForegroundColor Cyan
...
export class InvoicesService {
  ...
  async createGroupedInvoice(dto: CreateGroupedInvoiceDto) {
    ...
    for (const appt of appointments) {
      if (appt.intervention?.amount) {
        totalAmount += appt.intervention.amount;
      }
    }
...
Write-Host "Dossier : $baseDir" -ForegroundColor Cyan